...
def update_gr_general(outcome_var, indep_var, chk, yr):
	...
	# Display map
	dfplot1 = dfplot1[dfplot1['year'] == yr]
	if dfvar[dfvar['var'] == indep_var]['vartype'].values[0] == 'cat':
		fig2 = px.choropleth(dfplot1, locations='state_ab', locationmode='USA-states', scope='usa', color=indep_var, \
			color_discrete_map={'no': 'cornsilk', 'yes': 'cadetblue', 'na': 'whitesmoke'}, width=500)
	else:
		fig2 = px.choropleth(dfplot1, locations='state_ab', locationmode='USA-states', scope='usa', color=indep_var, \
			color_continuous_scale='deep', width=500)
	fig2.update_layout(
		plot_bgcolor='white',
		title={'text': dfvar[dfvar['var'] == indep_var]['descr'].values[0] + ' - ' + str(yr), 'xanchor': 'center', 'yanchor': 'top', 'x': 0.5, 'y': 0.9, \
...
app.layout = html.Div(
	[
		html.Table([
			...
			html.Tr([
				html.Td([
					dcc.Graph(id='gr_map1', style={'width': '500px'})
				], style={'border': 'none', 'padding': '0px'}),
				html.Td([
					dcc.Graph(id='gr_map2', style={'width': '500px'})
				], style={'border': 'none', 'padding': '0px'})
			], style={'border': 'none', 'padding': '1px'}),
			html.Tr([
				html.Td([
					dcc.Graph(id='gr_map3', style={'width': '500px'})
				], style={'font-size': '14px', 'border': 'none', 'padding': '1px'}),
			], style={'border': 'none', 'padding': '0px'}),
			html.Tr([
				html.Td([
					html.Div([
						dcc.Slider(id='sl_year', min=1975, max=2002, step=3, value=1975, marks=d_yr, updatemode='drag'),
						html.Button(id='but_play', children='Play', n_clicks=0, disabled=False),
						dcc.Interval(id="ani_map", disabled=True, interval=1000, n_intervals=0, max_intervals=-1)
					], style={'width': '75%', 'margin': 'auto'})
				], style={'border': 'none', 'padding': '1px', 'text-align': 'left'})
			], style={'border': 'none', 'padding': '1px'}),
			html.Tr([
				html.Td([
					html.Label([''])
				...
				html.Td([
					html.Table([
						...
						html.Tr([
							...
							html.Td([
								html.Div([
									dcc.Slider(id='sl_year2', min=1975, max=2002, step=3, value=1975, marks=d_yr, updatemode='drag'),
									html.Button(id='but_play2', children='Play', n_clicks=0, disabled=False),
									dcc.Interval(id="ani_map2", disabled=True, interval=1000, n_intervals=0, max_intervals=-1)
								], style={'width': '75%', 'margin': 'auto'})
							], style={'font-size': '14px', 'border': 'none', 'padding': '1px', 'height': '40px'}, colSpan=2)
						], style={'border': 'none', 'padding': '0px'}),
						html.Tr([
						...
						html.Tr([
							...
							html.Td([
								html.Div([
									dcc.Slider(id='sl_year3', min=1975, max=2002, step=3, value=1975, marks=d_yr, updatemode='drag'),
									html.Button(id='but_play3', children='Play', n_clicks=0, disabled=False),
									dcc.Interval(id="ani_map3", disabled=True, interval=1000, n_intervals=0, max_intervals=-1)
								], style={'width': '75%', 'margin': 'auto'})
							], style={'font-size': '14px', 'border': 'none', 'padding': '1px', 'height': '40px'}, colSpan=2)
						], style={'border': 'none', 'padding': '0px'}),
						html.Tr([
...
# Callback functions to update tis maps
@app.callback(
...
def update_tis_maps(sl_year):

	# Fetch data
	dfplot = df1[df1['year'] == sl_year]

	# Display map1 - tiss
	fig1 = px.choropleth(dfplot, locations='state_ab', locationmode='USA-states', scope='usa', color='tiss', \
		color_discrete_map={'no': 'cornsilk', 'yes': 'cadetblue', 'na': 'whitesmoke'}, width=500)
	fig1.update_layout(
		plot_bgcolor='white',
	...
	fig2 = px.choropleth(dfplot, locations='state_ab', locationmode='USA-states', scope='usa', color='tisviol_agg', \
		color_discrete_map={'no': 'cornsilk', 'yes': 'cadetblue', 'na': 'whitesmoke'}, width=500)
	fig2.update_layout(
		plot_bgcolor='white',
		title={'text': 'Mandatory sentencing for violent crime - ' + str(sl_year), 'xanchor': 'center', 'yanchor': 'top', 'x': 0.5, 'y': 0.9, \
			'font': {'family': 'Calibri', 'size': 18}}
	)

	# Display mean tisp by presence of determinate sentencing
	dfplot_mean = dfplot.groupby(['pre_par'], as_index=False).agg({'tisp': 'mean'})
	dfplot_mean.columns.values[1] = 'tisp'
	fig3 = px.bar(dfplot_mean, orientation='h', x='tisp', y='pre_par', height=180, width=500)
	fig3.update_layout(
		plot_bgcolor='white',
...
@app.callback(
	...
	Input('chk_dyn', 'value'),
	Input('sl_year2', 'value'),
	prevent_initial_call=False
)
def update_dyn_gr(outcome_var, indep_var, chk, yr):
	try:
		r1, r2, chk = update_gr_general(outcome_var, indep_var, chk, yr)
		return r1, r2, chk
	except:
		return {}, {}, {}
...
@app.callback(
	...
	Input('sl_year3', 'value'),
	prevent_initial_call=False
)
def update_gr_tiseff(outcome_var, chk_mean, mapyr):
	...
	if dfvar[dfvar['var'] == outcome_var]['vartype'].values[0] == 'cat':
		fig2 = px.choropleth(dfplot1, locations='state_ab', locationmode='USA-states', scope='usa', color=outcome_var, \
			color_discrete_map={'no': 'cornsilk', 'yes': 'cadetblue', 'na': 'whitesmoke'}, width=500)
	else:
		fig2 = px.choropleth(dfplot1, locations='state_ab', locationmode='USA-states', scope='usa', color=outcome_var, \
			color_continuous_scale='deep', width=500)
...
@app.callback(
	Output('but_play', 'children'),
	Output('ani_map', 'disabled'),
	Input('but_play', 'n_clicks'),
	prevent_initial_call=True
)
def enable_animate(but_play):
	if but_play % 2 == 0:
		return 'Play', True
	else:
		return 'Stop', False
...
@app.callback(
	...
	Input('ani_map', 'n_intervals')
)
def animate_map(sl_year, ani_map, n_int):
	newyr = sl_year
	if not ani_map:
		newyr += 3
...
# Callback functions for play/stop button (dynamic maps)
@app.callback(
	Output('but_play2', 'children'),
	Output('ani_map2', 'disabled'),
	Input('but_play2', 'n_clicks'),
	prevent_initial_call=True
)
def enable_animate_dyn(but_play):
	if but_play % 2 == 0:
		return 'Play', True
	else:
		return 'Stop', False
...
# Callback functions to animate map (dynamic maps)
@app.callback(
	Output('sl_year2', 'value'),
	Input('sl_year2', 'value'),
	Input('ani_map2', 'disabled'),
	Input('ani_map2', 'n_intervals')
)
def animate_map_dyn(sl_year, ani_map, n_int):
	newyr = sl_year
	if not ani_map:
		newyr += 3
		if newyr > 2002: newyr = 1975
...
# Callback functions for play/stop button (tis effects)
@app.callback(
	Output('but_play3', 'children'),
	Output('ani_map3', 'disabled'),
	Input('but_play3', 'n_clicks'),
	prevent_initial_call=True
)
def enable_animate_tiseff(but_play):
	if but_play % 2 == 0:
		return 'Play', True
	else:
		return 'Stop', False
...
# Callback functions to animate map (tis effects)
@app.callback(
	Output('sl_year3', 'value'),
	Input('sl_year3', 'value'),
	Input('ani_map3', 'disabled'),
	Input('ani_map3', 'n_intervals')
)
def animate_map_tiseff(sl_year, ani_map, n_int):
	newyr = sl_year
	if not ani_map:
		newyr += 3
		if newyr > 2002: newyr = 1975
